...
<head>
	...
	<body>
	<div class="functionhead"><i18n:label localeRef="mylocale" code="<%=SupplierMessageTag.SUPPLIER_LISTING%>"/></div>
...
<% if (canView) { %>    
	<table class="listbox" width="100%">
		  ...
		  <tr class="boxhead" valign=top>
			...
			<td><i18n:label localeRef="mylocale" code="<%=SupplierMessageTag.SUPPLIER_CODE%>"/></td>
			...
			<td><i18n:label localeRef="mylocale" code="<%=StandardMessageTag.CONTACT_INFORMATION%>"/></td>
			<td></td>
		  </tr>
		  ...
		  %>
			   <tr class="<%= rowCss %>" valign=top>
					...
					<td>
						<a href="<%=Sys.getControllerURL(SupplierManager.TASKID_SUPPLIER_INFO,request)%>&scode=<%=scode%>">
					...
					<td>
						<a href="<%=Sys.getControllerURL(SupplierManager.TASKID_SUPPLIER_EDIT_FORM,request)%>&scode=<%=scode%>">
