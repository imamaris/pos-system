...
<%@ include file="/lib/header.jsp" %>
	...
	<body>
	<div class="functionhead">Task Status Settings</div>
 	<form method="post" name=tasklist action="" action="<%=Sys.getControllerURL(ActionTaskManager.TASKID_TASK_INDIVIDUAL_TASK_SETTINGS,request)%>">
	...
	<table class="listbox"  width="90%">
		  ...
		  %>
			   ...
			   <tr class="<%= ((++count)%2 == 0) ? "even" : "odd"%>" valign=top>
					...
					<td><%=((beans[i].getStatusmsg() != null) ? beans[i].getStatusmsg() : "")%></td>
					<td align=center><small><std:link text="Update" taskid="<%=ActionTaskManager.TASKID_TASK_INDIVIDUAL_TASK_UPDATE%>" params="<%=("taskid="+beans[i].getTaskID())%>" /></small></td>
			  </tr>
		  <% } %>
