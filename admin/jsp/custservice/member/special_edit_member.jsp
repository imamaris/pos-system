  ...
  <script language="javascript">
    	...
    	if (thisform.NationalityID != null) {
  		...
  		if (confirm('<i18n:label code="MSG_PROCEED_CHANGE"/>')) {
				thisform.action = "<%=Sys.getControllerURL(MemberManager.TASKID_SPECIAL_EDIT_MEMBER,request)%>";		
				thisform.submit();
			}
		}
...
<div class="functionhead"><i18n:label localeRef="mylocale" code="<%=MemberMessageTag.SPECIAL_EDIT_DIST%>"/> - <i18n:label localeRef="mylocale" code="<%=MemberMessageTag.NAME_BANK_INFO%>"/>
</div>

<form name="frmSearch" action="<%=Sys.getControllerURL(MemberManager.TASKID_SPECIAL_EDIT_MEMBER,request)%>" method="post">
...
<form name="frmEdit" action="" method="post">
	...
	<table class="tbldata" width="100%">
	  ...
	  <tr valign="top">
		...
		<tr>
			...
			<td>
				<std:input type="radio" name="Gender" value="M" status="<%= bean.getGender() != null && bean.getGender().equals("M") ? "checked" : "" %>"/>&nbsp; Male &nbsp;&nbsp;&nbsp;
	      <std:input type="radio" name="Gender" value="F" status="<%= bean.getGender() != null && bean.getGender().equals("F") ? "checked" : "" %>"/>&nbsp; Female
	    </td>
		</tr>
		<tr>
			<td align="right"><span class="required note">* </span><i18n:label localeRef="mylocale" code="<%=StandardMessageTag.MARRIAGE%>"/>:</td>
			<td>
				<std:input type="radio" name="Marital" value="M" status="<%= bean.getMarital() != null && bean.getMarital().equals("M") ? "checked" : "" %>"/>&nbsp; Yes &nbsp;&nbsp;&nbsp;
				<std:input type="radio" name="Marital" value="S" status="<%= bean.getMarital() != null && bean.getMarital().equals("S") ? "checked" : "" %>"/>&nbsp; No
	    </td>
		</tr>
		<tr>
  ...
  <input type="hidden" name="OldName" value="<%= bean.getName() %>">
  <input type="hidden" name="OldBankID" value="<%= (payeeBank.getBankBean() != null && payeeBank.getBankBean().getName() != null) ? payeeBank.getBankBean().getName() : "-" %>">
  <input type="hidden" name="OldBankPayeeName" value="<%= (payeeBank.getBankPayeeName() != null) ? payeeBank.getBankPayeeName() : "-" %>">
  <input type="hidden" name="OldBankAcctNo" value="<%= (payeeBank.getBankAcctNo() != null) ? payeeBank.getBankAcctNo() : "-" %>">
  <input type="hidden" name="OldBankBranch" value="<%= (payeeBank.getBankBranch() != null) ? payeeBank.getBankBranch() : "-" %>">
