...
<%
  ...
  boolean canView = network != null && network.getRoot() != null && network.getDownlineLevels() > 0;
%>
...
<div class="functionhead"><i18n:label localeRef="mylocale" code="<%=MemberMessageTag.DOWNLINE_LISTING%>"/></div>

<form name="frmSearch" action="<%=Sys.getControllerURL(SponsorTreeManager.TASKID_DOWNLINE_LISTING,request)%>" method="post">
...
<table>
  ...
  <tr valign="top">
  	<td align="right"><i18n:label localeRef="mylocale" code="<%=MemberMessageTag.TOTAL_DOWNLINES%>"/> <br> (<i18n:label localeRef="mylocale" code="<%=MemberMessageTag.EXCLUDE_SELF%>"/>) </td>
    <td>:</td>
    <td><%= network.getDownlines() - 1 %></td>
	</tr>
</table>
...
<%
	ArrayList list = network.getDownlineLevelList();
...
%>
	...
	<tr class="boxhead">
		<td colspan="6" align="left"><font color="blue"><b><i18n:label localeRef="mylocale" code="<%=MemberMessageTag.LEVEL%>"/> (<%= i %>) <i18n:label code="GENERAL_TOTAL"/>: <%= nodes.length %></b></font></td>
	</tr>
