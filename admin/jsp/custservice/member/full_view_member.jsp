...
<div class="functionhead"><i18n:label localeRef="mylocale" code="<%=MemberMessageTag.VIEW_DIST%>"/> - <i18n:label localeRef="mylocale" code="<%=MemberMessageTag.FULL_INFO%>"/></div>
...
<input class="textbutton noprint" type="button" value="CUSTOMER SALES ENTRY" onClick="location.href='<%= Sys.getControllerURL(CounterSalesManager.TASKID_NORMAL_SALES_PREFORM_FULL_DELIVERY_HE,request) %>&CustomerContact=<%= newBean.getMobileNo() %>&CustomerID=<%= newBean.getMemberID() %>&CustomerName=<%= newBean.getName() %>&CustomerAlamat=<%= alamat %>&CustomerSegmentation=<%= newBean.getSegmentationCRM() %>'">            
