...
<table class="listbox" width="100%" >
  ...
  %>
    ...
    <td nowrap align="center"><a href="<%= Sys.getControllerURL(CounterSalesManager.TASKID_NORMAL_SALES_PREFORM_FULL_DELIVERY_HE_FORCE,request) %>&CustomerContact=<%= beans[i].getMobileNo() %>&CustomerID=<%= beans[i].getMemberID() %>&CustomerName=<%= beans[i].getName() %>">  <%= beans[i].getMemberID() %> </a> </td> 
    <td nowrap align="center"><a href="javascript:window.opener.document.getElementById('custID').value='<%= beans[i].getMemberID() %>'; window.opener.document.getElementById('custContact').value='<%= beans[i].getMobileNo() %>'; window.opener.document.getElementById('NmCust').value='<%= beans[i].getName() %>'; window.close(); window.opener.document.getElementById('submit').disabled = false; window.opener.document.getElementById('submit').focus(); " > <%= beans[i].getMemberID() %> </a> </td> 
