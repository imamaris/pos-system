...
<table class="listbox" width="100%" >
  <tr class="boxhead" valign=top>
    ...
    <td width="10%">Segment</td> <!-- Updated By Ferdi 2014-09-04 -->
  </tr>
  ...
  <%
      ...
      if (beans[i].getStatus() != MemberManager.MBRSHIP_ACTIVE)
        ...
        if (beans[i].getSegmentationCRM() != null && beans[i].getSegmentationCRM().trim().length() > 0) Segment = "Segment : <b>" + beans[i].getSegmentationCRM() + "</b>";
        //
  %>
    ...
    <td width="5%"><%= i+1 %>.</td>
    <td nowrap align="center"><a href="javascript:window.opener.document.getElementById('custID').value='<%= beans[i].getMemberID() %>'; window.opener.document.getElementById('custContact').value='<%= beans[i].getMobileNo() %>'; window.opener.document.getElementById('NmCust').value='<%= beans[i].getName() %>'; window.opener.document.getElementById('SegmentationCrm').value='<%= beans[i].getSegmentationCRM() %>'; window.opener.document.getElementById('custSegment').innerHTML='<%= Segment %>'; window.close(); window.opener.document.getElementById('submit').disabled = false; window.opener.document.getElementById('submit').focus(); " > <%= beans[i].getMemberID() %> </a> </td> <!-- Updated By Ferdi 2014-01-22 -->
    ...
    <td align="center"><b><%= beans[i].getSegmentationCRM() %></b></td> <!-- Updated By Ferdi 2014-09-04 -->
  </tr>
