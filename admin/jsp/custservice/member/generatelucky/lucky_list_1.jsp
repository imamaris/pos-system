...
<%
	...
	Map register = (Map) returnBean.getReturnObject(MemberLuckyManager.RETURN_REGISTER_CODE);        
...
<table border="0" class="noprint">
	...
	<tr>		
		<td class="td1">Type Register:</td>
		<td><std:input type="select" name="RegisterID" options="<%=register %>" /></td>
		<td>&nbsp;</td>
		<td class="td1"></td>
		<td>&nbsp;</td>
	</tr>        
...
<c:when test="<%= showResult %>">
	...
	<tr class="<%= rowCss %>" valign=top>
		...
		<td align=center nowrap><%= MemberLuckyManager.defineRegisterType(beans[i].getReferenceNo())%></td>                
                ...
                <%}%>
		<td nowrap>
		<%if(beans[i].getStatus() == MemberLuckyManager.STATUS_UNUSED){%>
		  	<font color="blue"><%= (MemberLuckyManager.defineIDStatus(MemberLuckyManager.STATUS_UNUSED)) %></font>
		<%}else{ %>
		  	<font color="red"><%= (MemberLuckyManager.defineIDStatus(MemberLuckyManager.STATUS_USED)) %></font>
		<%}%>
		</td>
	</tr>
	<%
