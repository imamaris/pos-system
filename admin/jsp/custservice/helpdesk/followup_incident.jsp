...
<form name="frmIncidentFollow" action="<%=Sys.getControllerURL(HelpdeskManager.TASKID_HELPDESK_INCIDENT_FOLLOW,request)%>" method="post">
	...
	<table class="listbox" width="100%" cellspacing="0">
		<tr class="boxhead" valign=top align="center">
		  ...
		  %>
				...
				<td>
					<img border="0" alt='Follow-up' src="<%= Sys.getWebapp() %>/img/icon_edit.gif" onClick="return popitup('<%=Sys.getControllerURL(HelpdeskManager.TASKID_HELPDESK_INCIDENT_FOLLOW_ADD,request)%>'+'&id='+'<%=String.valueOf(incidentBeans[i].getSeq())%>')"/>
				</td>
			  </tr>	
		  <% } // end for loop %>
	</table>
