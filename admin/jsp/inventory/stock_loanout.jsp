...
function validateQty(idx, qty) {
 ...
 function searchInventory(){
  ...
  if(confirm('<i18n:label code="MSG_CONFIRM"/>'))  {
     myform.action = '<%=Sys.getControllerURL(InventoryManager.TASKID_STOCK_PRE_LOANOUT, request)%>%>';
     myform.submit();
  }else{
...
<div class=functionhead><i18n:label code="STOCK_LOANOUT"/></div>

<form name="ops_inventory" action="<%=Sys.getControllerURL(InventoryManager.TASKID_STOCK_LOANOUT, request)%>" method="post">
...
<table>
	...
	<tr valign=top>
		<td><b><i18n:label code="GENERAL_GIVETO"/></b></td>
		<td>: 
...
<table class="listbox" width="70%">
  	<tr class="boxhead" valign=top>
  		...
  		<td align=right width="8%"><i18n:label code="STOCK_QTY_LOAN"/></td>
