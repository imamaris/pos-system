...
function validateQty(idx, qty) {
 ...
 function sumTotalQuantity(){
   ...
   if(thisform.qty.length !=  null){
    ...
    if(counter > 0 && thisform.storeTo.length > 0 )
       offButton(false);
    else
       offButton(true);      
 }
 ...
 function searchInventory(){
  ...
  var myform = document.ops_inventory;
	  ...
	  if(confirm('<i18n:label code="MSG_CONFIRM"/>'))  {
	     myform.action = '<%=Sys.getControllerURL(InventoryManager.TASKID_STOCK_PRE_TRANSFER_INTERNAL_2, request)%>%>';
	     myform.submit();
	  }else{
	     myform.id.value = myform._id.value;
             myform.docno.value = myform._docno.value;
	  }    
 }
...
<%
  ...
  String docno = (String) returnBean.getReturnObject("DOCNO");
  ...
  InventoryBean inventory = (InventoryBean) returnBean.getReturnObject("Inventory");
  TreeMap inventorys = (TreeMap) returnBean.getReturnObject("Inventorys");
...
<div class=functionhead><i18n:label code="Stock Transfer II (Internal)"/></div>

<form name="ops_inventory" action="<%=Sys.getControllerURL(InventoryManager.TASKID_STOCK_TRANSFER_INTERNAL_2, request)%>" method="post">
...
<table>
	...
	<tr>
		<td colspan=2><b><i18n:label code="STOCK_TRANSFER_STORE"/></b></td>
	</tr>
	<tr>
		...
		<td>: 
        	<std:input type="select" name="docno" options="<%=inventorys%>" value="<%=inventory.getTrnxDocNo()%>" status="onChange=searchInventory();"/>                 

                <b><%=docno%></b><input type=hidden name="_docno" value="<%=inventory.getTrnxDocNo()%>">
	...
	<tr>
		<td align=right><i18n:label code="GENERAL_TO"/></b></td>
		<td>: 
...
%>     
     ...
     <tr class=<%=((i%2==0)?"odd":"even")%> valign=top>
		...
		<td align=right><input type=text name="qty" maxlength=8 size=5 style="text-align:right" value="<%=invenBeans[i].getTotal()%>" onKeyUp="validateQty( <%=i%>, this)"></td>
