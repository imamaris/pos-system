...
function validateQty(idx, qty) {
 ...
 function searchInventory(){
  ...
  if(confirm('<i18n:label code="MSG_CONFIRM"/>'))  {
     myform.action = '<%=Sys.getControllerURL(InventoryManager.TASKID_STOCK_PRE_LOANRETURN, request)%>%>';
     myform.submit();
  }else{
...
<div class=functionhead><i18n:label code="STOCK_LOANRETURN"/></div>

<form name="ops_inventory" action="<%=Sys.getControllerURL(InventoryManager.TASKID_STOCK_LOANRETURN, request)%>" method="post">
...
<table>
	...
	<tr valign=top>
		...
		<td>: 
		<textarea name="TargetRemark" rows="4" cols="40"></textarea>
        </td>
	</tr>
</table>
<br>
<table class="listbox" width="70%">
  	<tr class="boxhead" valign=top>
  		...
  		<td align=right width="8%"><i18n:label code="STOCK_QTY_RETURN"/></td>
...
%>             
     <tr class=<%=((i%2==0)?"odd":"even")%> valign=top>
  		...
  		<td align=right><input type=text name="qty_in" maxlength=8 size=5 style="text-align:right" onKeyUp="validateQty( <%=i%>, this)"></td>
