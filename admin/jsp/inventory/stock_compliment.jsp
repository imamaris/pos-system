...
function validateQty(idx, qty) {
 ...
 function searchInventory(){
  ...
  if(confirm('<i18n:label code="MSG_CONFIRM"/>'))  {
     myform.action = '<%=Sys.getControllerURL(InventoryManager.TASKID_STOCK_PRE_COMPLIMENT, request)%>%>';
     myform.submit();
  }else{
...
<div class=functionhead><i18n:label localeRef="mylocale" code="<%=InventoryMessageTag.STOCK_COMPLIMENT%>"/></div>

<form name="ops_inventory" action="<%=Sys.getControllerURL(InventoryManager.TASKID_STOCK_COMPLIMENT, request)%>" method="post">
...
<table>
	...
	<tr valign=top>
		<td><b><i18n:label code="STOCK_COMPLIMENT_TO"/></b></td>
		<td>: 
