...
function validateQty(idx, qty) {
  ...
  if (unit.length > 0){ 
	 ...
	 if (unit > bal){
	     ...
	     alert("Quantity is greater from loan stock ");
...
// function showEmp1(i, tanggal, lokasi, kategori, customer, sku_kode)

function showEmp1(i, tanggal, lokasi, customer, sku_kode)
{ 
// var echo = 0;
if(document.getElementById("icode_"+i).value!="-1")
{
    ...
    url=url+"?sku_kode="+sku_kode+"&tanggal="+tanggal+"&lokasi="+lokasi+"&customer="+customer+"&echo=" + echo;
    echo = echo + 1;       
 ...
 xmlHttp.onreadystatechange=function() {
    if (xmlHttp.readyState==4) {
      ...
      stateChanged(i, tanggal, lokasi, customer, xmlHttp.responseText);      
    }
  };
...
function stateChanged(i, tanggal, lokasi, customer, text) 
{ 
        // alert(tanggal);
            ...
            <td>                                    
                ...
                <table class="listbox" width="550">
                    ...
                    <tr >
                        ...
                        <td >
                            <input type="text" name="icode_<%= i%>" id="icode_<%= i%>" value="" onKeyUp="showEmp1(<%= i%>, '<%= Sys.getDateFormater().format(new Date()) %>', '<%= outletFrom.getOutletID() %>', '<%= custID %>', this.value);" onblur="showKit2(<%= i%>,  '<%= Sys.getDateFormater().format(new Date()) %>', '<%= outletFrom.getOutletID() %>', this.value);"  size="10" maxlength="50" >                                                     
