...
<head>
	...
	function calcKIV(idx) {
		...
		var qtyLeft = document.frmDelivery.PrdtQtyLeft[idx];
                var qtyOnHand = document.frmDelivery.PrdtQtyOnHand[idx];
		var kiv = PrdtKIV[idx];
		...
		if (qtyOnHand == null) {
			qtyOnHand = document.frmDelivery.PrdtQtyOnHand;
		}
		...
		if (qtyOnHand.value - qty.value < 0 ) {
	...
	function calcTotalQty() {
		...
		if (thisform.PrdtQtyIssued.type == "text") {
			...
			totalKiv += PrdtKIV.innerText * 1;
                        totalQtyOnHand += thisform.PrdtQtyOnHand.value * 1;
			...
			for (var i=0; i < thisform.PrdtQtyIssued.length; i++) {
				...
				totalKiv += PrdtKIV[i].innerText * 1;
                                totalQtyOnHand += thisform.PrdtQtyOnHand[i].value * 1;
			}
		...
		<td colspan="2" width="700">
		  <table class="listbox" width="800">
				...
				<tr class="even" valign=top>
				  ...
				  <td align="right"><input type=text name="PrdtQtyIssued" value="0" size=5 style="text-align:right" onKeyUp="calcKIV(<%= pdCnt %>)"></td>
				...
				<std:input type="hidden" name="PrdtQtyOnHand" value="<%= cpb[j].getQtyOnHand() %>"/> 
