...
<html>
    ...
    <body>
        ...
        <div class="functionhead">Sales Void</div>
        
        <form name="frmSearch" action="<%=Sys.getControllerURL(CounterSalesManager.TASKID_SALES_LIST_VOID,request)%>" method="post" onSubmit="return doSubmit(document.frmSearch);">
        ...
        <form name="frmList" action="" method="post">
            ...
            <table class="listbox" width="80%">
                ...
                <tr class="<%= rowCss %>" valign=top>
                    ...
                    <td align="center">
                        <%      if ( isNew ) {    %>
                        <small><std:link text="<%=("<li>" + lang.display("Process by Admin"))%>" taskid="<%= CounterSalesManager.TASKID_ADMIN_VOID_SALES %>" params="<%=("SalesID="+beans[i].getSalesID())%>" /></small>
                        <%      } %>	
                        <small><std:link text="<%=("<li>" + lang.display("Process by Super"))%>" taskid="<%= CounterSalesManager.TASKID_SUPER_VOID_SALES %>" params="<%=("SalesID="+beans[i].getSalesID())%>" /></small>
                    </td>
