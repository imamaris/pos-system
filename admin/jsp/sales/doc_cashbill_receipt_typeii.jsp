...
%>
<title>Cash Bill Receipt for Chi Network</title>
</head>
...
<c:if test="true">
		...
		<div align=left>Chi Network	</div>
          <hr noshade>
          </td>
	</tr>
	<tr class=printreceipt valign=top> 
...
<%
	...
	for (int i = 0; i < itemSales.length; i++) {
		...
		gross += (itemSales[i].getQtyOrder() * unitType);
%>
	...
	<tr class=printreceipt>
		<td width=40% nowrap><%= String.valueOf(itemSales[i].getQtyOrder()) + " x "%><std:currencyformater code="" value="<%= unitType %>"/></td>   
		<td align="right"><std:currencyformater code="" value="<%= itemSales[i].getQtyOrder() * unitType %>"/></td>   
	</tr>  
...
<table width="<%=width%>">
  ...
  %>
	...
	<tr class=printreceipt>
		<td width=40%><i18n:label code="SALES_NET"/></td>  
		<td nowrap>: <%=bean.getLocalCurrencySymbol()%></td>   
		<td align="right"><std:currencyformater code="" value="<%=(bean.getChiSalesAmount())%>"/></td>   
 	</tr>
<% 
	CounterSalesPaymentBean[] paymentList = bean.getPaymentArray();
	
	for (int m = 0; m < 0/*paymentList.length*/; m++) { 
