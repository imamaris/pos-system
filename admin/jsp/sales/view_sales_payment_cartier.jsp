<table width="650" border="0" cellpadding="0" cellspacing="0">	
	...
	%>         
		...
		<td valign="top" align="right" nowrap width="2%">  <%= payment.getCurrency()%> </td>
                ...
                <% if (payment.getCurrency().equalsIgnoreCase("IDR"))
                    {                
                %>
                <td valign="top" align="right" nowrap width="5%"></td>
                <%
                } else {
                %>                
                <td valign="top" align="right" nowrap width="5%"> <std:currencyformater code="" value="<%= payment.getAmount() %>"/> </td>
                <%
                }   // end if
                %>                 
                ...
                <%                  
                 if (payment.getRefNo().trim() != null && payment.getRefNo().length() > 0 && panjang > 12)
                 {    
                    ...
                    // bayar = "test"; 
                    bayar = payment.getRefNo().substring(0,4).concat("-").concat(payment.getRefNo().substring(4,8)).concat("-").concat(payment.getRefNo().substring(8,12)).concat("-").concat(payment.getRefNo().substring(12,panjang));                                          
                    }    
