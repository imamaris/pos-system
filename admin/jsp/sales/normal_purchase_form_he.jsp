...
{      
  ...
  if ( y1 == "CASH")
	...
	var form_valid=document.getElementById('valValid'+i);
        ...
        var form_card=document.getElementById('valCard'+i);
        var form_amount1=document.getElementById('amount_'+i);
        var form_amount2=document.getElementById('amount2_'+i);
	...
	var hidden=form.value;
        ...
        b=form.value.replace("<STX>","").replace("<CR>","").replace("<ETX>","").split("^");        
            ...
            if (form_valid.value.length < 10)                
            {    
                $.post("voucher.jsp",{"productSKU" : sku_kode},function(data){
                    var voucher = trim(data).split(";");

                    form_valid.value="2013-12-31";                 
                    form_nama.value=voucher[2];                        
                    form_card.value= voucher[5]; 
                    form_amount1.value= voucher[8];
                    form_amount2.value= voucher[8];
                });
            }    
        }                     
}
        ...
        <form name="frmSalesOrder" action="" method="post">
            ...
            <table width="80%" border="0">   
                ...
                <tr>		                   
                    ...
                    <input type="hidden" name="amount_<%= i%>" id="amount_<%= i%>" value=""/>
                    <td colspan="2">
                    ...
                    <td >
                        <input type="text" name="refNo_<%= i%>" id="scanVal<%= i%>" onKeyUp="split_char(<%= i%>);" onblur="calcGrandTotal2()" />                        
                    ...
                    <td colspan="2">
                        <input size="32" type="text" name="owner_<%= i%>" id="owner_<%= i%>" value="" disabled />   
                    ...
                    <td >
                        <input type="text" name="refNo_<%= i%>" id="scanVal<%= i%>" onkeyup="Voucher(<%= i%>, this.value);"/>          
                    </td>                                                                                
                    <td align="right">                        
                        <input type="text" name="<%= "amount_" + i %>"  onKeyUp="calcAmountPaid(this, <%= i%>)" />  
                    </td>    
                    <td colspan="2">
                        <input size="32" type="text" name="owner_<%= i%>" id="valName<%= i%>" value="" />                        
                    </td>                     
                    <td align="center">                        
                        <input size="10" type="text" name="expired_<%= i%>" id="valValid<%= i%>"  onblur="valDate(this);"  />                   
                    </td>                    
                </tr>                                                             
                <input type="hidden" name="amount2_<%= i%>" id="amount2_<%= i%>" value="" disabled />                                         
                <input type="hidden" name="tempVal<%= i%>" id="tempName<%= i%>" value=" "/>                           
                <input type="hidden" class="boxcontent" name="card_<%= i%>" id="valCard<%= i%>" value="" disabled />                                
                <%
                                } // loop 5x
