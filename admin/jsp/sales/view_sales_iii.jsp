...
<body>
<div class="functionhead"><i18n:label code="SALES_ORDER_VIEW"/> for Corporate</div>
...
<table width="100">
	<tr>
		<td>
			<input class="noprint textbutton" type="button" value="<i18n:label code="GENERAL_BUTTON_PRINT_RECP"/>" onClick="javascript:popupViewSmallReceipt('<%=Sys.getControllerURL(CounterSalesManager.TASKID_DOC_CB_RECEIPT_TYPEIII,request) %>&SalesID=<%= bean.getSalesID() %>')">
		</td>
		<!--td>
			<input class="noprint textbutton" type="button" value="<i18n:label code="GENERAL_BUTTON_PRINT_COMP"/>" onClick="javascript:popupViewDoc('<%=Sys.getControllerURL(CounterSalesManager.TASKID_DOC_CB_TYPEIII,request) %>&SalesID=<%= bean.getSalesID() %>')">
		</td-->
