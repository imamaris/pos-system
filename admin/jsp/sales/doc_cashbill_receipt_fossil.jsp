...
<table width="<%=width%>">
	...
	<tr class=printreceipt valign=top> 
	  <td >Location : </td>
	  <td colspan="2" align=right><%=(bean.getSellerID())%></td>
	</tr>
        <tr class=printreceipt valign=top> 
	  <td >Sales Invoice : </td>
        ...
        <% if (itemSales[i].getUnitDiscount() > 0) 
            {
                    %>
	<tr class=printreceipt>		
            <td colspan=3 valign=top> <%= tabLine + tabLine + tabLine + String.valueOf( (itemSales[i].getUnitDiscount() / itemSales[i].getUnitPrice()) * 100 ).substring(0,3) + " % " %> ( <std:currencyformater code="" value="<%= itemSales[i].getUnitPrice()%>"/> )</td>  
	</tr>  
        <%  } %>
