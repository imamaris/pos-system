...
<form name="frmSearch" action="<%=Sys.getControllerURL(CounterSalesManager.TASKID_SALES_LIST,request)%>" method="post" onSubmit="return doSubmit(document.frmSearch);">
	  ...
	  <tr>
	    <td class="td1">Customer ID:</td>
            <td><std:input type="text" name="CustomerID" size="20" maxlength="20"/></td>                   
	    <td>&nbsp;</td>
	  ...
	  <tr>
	    ...
	    <td>&nbsp;</td>
            <td class="td1">Sales Person ID:</td>          
	    ...
	    <td><std:input type="date" name="TrxDateFrom" value="now"/> <br>(yyyy-mm-dd)</td>
	    <td>&nbsp;</td>
	    <td class="td1"><i18n:label code="SALES_TRX_DATE"/> <i18n:label code="GENERAL_TO"/>:</td>
	    <td><std:input type="date" name="TrxDateTo" value="now"/> <br>(yyyy-mm-dd)</td>
	  </tr>
          <tr>
	  	<td class="td1">Doc Date From:</td>
	    <td><std:input type="date" name="DocDateFrom" value="now"/> <br>(yyyy-mm-dd)</td>
	    <td>&nbsp;</td>
	    <td class="td1">Doc Date To:</td>
	    <td><std:input type="date" name="DocDateTo" value="now"/> <br>(yyyy-mm-dd)</td>
	  </tr>          
	  <tr>
...
<table class="listbox" width="100%">
  <tr class="boxhead" valign=top>
    ...
    <td align="center" nowrap ><i18n:label code="SALES_TRX_DATE"/> <br>(dd-mm-yyyy)</td>
    <td align="center" nowrap >Doc. Date <br>(dd-mm-yyyy)</td>
  ...
  <%
  	...
  	for (int i=0; i<beans.length; i++) { 
		...
		if (beans[i].getSellerTypeStatus().equalsIgnoreCase(CounterSalesManager.TYPE_STOCKIST)) {
      	...
      	if (isActive) {
	  		grdTotalBV += beans[i].getTotalBv1();
                        ...
                        grdVoucher += beans[i].getDiscountAmount();
        ...
        java.util.Date hari_ini = new Date();
        // new Date();
        java.util.Date hari_lalu = beans[i].getBonusDate();
        
        jumlah =  hari_ini.compareTo(hari_lalu);  
        
        CounterSalesManager csoMgr = new CounterSalesManager();
        String emailStatus = csoMgr.getEreceiptStatus(beans[i].getSalesID());
  %>
    ...
    <td nowrap>
    	<small><std:link text="<%= beans[i].getTrxDocNo() %>" taskid="<%= CounterSalesManager.TASKID_VIEW_SALES %>" params="<%=("SalesID="+beans[i].getSalesID())%>" /></small>
            <font color="blue"> <b> <%= beans[i].getServiceAgentType() %> </b> </font>
    </td>
	  <td nowrap>
	  ...
	  <td nowrap>
	  	<std:text value="<%= beans[i].getServiceAgentID() %>"/>
	  </td>          
    <td align="center" nowrap><fmt:formatDate pattern="dd-MM-yyyy" type="both" value="<%= beans[i].getTrxDate() %>" /></td>
    <td align="center" nowrap><fmt:formatDate pattern="dd-MM-yyyy" type="both" value="<%= beans[i].getBonusDate() %>" /></td>
    ...
    <td align="right" nowrap><std:currencyformater code="" value="<%= beans[i].getDiscountAmount() %>"/></td>
    ...
    <td align="center">
    <%      if ( isNew ) {    %>
    			<small><std:link text="<%=("<li>" + lang.display("Void"))%>" taskid="<%= CounterSalesManager.TASKID_ADMIN_VOID_SALES %>" params="<%=("SalesID="+beans[i].getSalesID())%>" /></small>
    <%      } %>	    	
    </td>
	...
	<tr class="totalhead">
		<td  colspan="6" align="right"><i18n:label code="GENERAL_GRAND_TOTAL"/> : </td>
		<td nowrap align="right"><b><std:currencyformater code="" value="<%= grdGrossSales %>"/></b></td>
                <td nowrap align="right"><b><std:currencyformater code="" value="<%= grdDeposit %>"/></b></td>
                <td nowrap align="right"><b><std:currencyformater code="" value="<%= grdVoucher %>"/></b></td>
