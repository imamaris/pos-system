...
<title></title>
    ...
    <!--
  	function doSubmit() {
		...
		myform.action="<%=Sys.getControllerURL(CounterSalesManager.TASKID_SALES_LIST_TYPEIII,request)%>";
		myform.target='';
    	myform.submit();
  	}      	 
  	function doPrint() {
		...
		var myform = document.frmSearch;
		myform.action="<%=Sys.getControllerURL(CounterSalesManager.TASKID_SALES_LIST_TYPES_PRINT,request)%>&print=corp";
...
<div class="functionhead"><i18n:label code="SALES_LISTING"/> for Corporate</div>
  ...
  <%
	    	...
	    	for (int i=0; i<beans.length; i++) { 
	        	...
	        	if (isActive) {
		  	  		...
		  	  		grdNetSales += beans[i].getCorpSalesAmount();
    ...
    <td nowrap><%=i+1%>.</td>
    <td nowrap>
    	<small><std:link text="<%= beans[i].getTrxDocNo() %>" taskid="<%= CounterSalesManager.TASKID_VIEW_SALES_TYPEIII %>" params="<%=("SalesID="+beans[i].getSalesID())%>" /></small>
    </td>
	  <td nowrap>
    ...
    <td align="right" nowrap><std:currencyformater code="" value="<%= beans[i].getCorpSalesAmount() %>"/></td>
