...
try{
    int time = getHourNow();
    String left = left(custContact,6);
    hitungSys = rumusPassSalesForce(left,time);
...
<%!
String left(String mobile, int length){
    return mobile.substring(1,length);
}
    
int rumusPassSalesForce(String mobile, int jam){
    double dMobile = Double.parseDouble(mobile);
    double round = (dMobile + jam)/jam;
...
<std:input type="hidden" name="PriceCode" value="RTL" />
        ...
        <tr>
            ...
            <td><input type="hidden" name="pengSys" value="<%= hitungSys %>" /><input type="text" name="password" /></td>
        </tr>
