...
<%
  ...
  Map expeditionMap = (Map) returnBean.getReturnObject(CounterSalesManager.RETURN_EXPEDITIONLIST_CODE);
...
<form name="frmSalesOrder" action="<%=Sys.getControllerURL(CounterSalesManager.TASKID_STOCKIST_SALES_PREFORM,request)%>" method="post" onSubmit="return doSubmit(document.frmSalesOrder);">
	...
	<table class="tbldata">
		...
		<tr>
			<td class="td1"><span class="required note">* </span>Expedition Name:</td>
			<td><std:input type="select" name="ExpeditionID" options="<%= expeditionMap %>" /></td>
		</tr>
