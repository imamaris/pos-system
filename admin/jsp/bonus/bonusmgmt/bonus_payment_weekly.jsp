...
%>
		   <tr class="<%=((i%2==1)?"odd":"even")%>" >
		  		<td><%=(i+1)%></td>
		  		<td><std:link taskid="<%=BonusMasterReportManager.TASKID_UPDATE_BONUS_PAYMENT%>" text="<%=beans[i].getMemberID()%>" params="<%="memberid="+beans[i].getMemberID()+"&periodid="+periodid%>" /></td>
		  		...
		  		<td align="center" nowrap><%=beans[i].getPaymentRemark()%></td>
                                <td align=center><small><std:link text="<%= lang.display("GENERAL_BUTTON_UPDATE") %>" taskid="<%=BonusMasterReportManager.TASKID_UPDATE_BONUS_PAYMENT_SUBMIT%>" params="<%="memberid="+beans[i].getMemberID()+"&periodid="+periodid%>" /></small></td>                                
                            </tr>	
