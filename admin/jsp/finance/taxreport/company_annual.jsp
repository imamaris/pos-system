...
<head>
	...
	<body>
	<div class="functionhead">Company Annual Income Tax Report  (Tax Art 23 Report)</div>
 	<form method="post" name="taxlist" action="<%=Sys.getControllerURL(BonusMasterReportManager.TASKID_ADMIN_ANNUAL_COMPANY_TAX_REPORT,request)%>">
...
<table class="listbox" width="95%">
		  <tr class="boxhead">
		  		...
		  		<td align="center">Annual<br>Tax<br>Paid</td>
		  </tr>
...
<%		  
			...
			for (int i=0 ; i<beans.length; i ++) {
				...
				subtotal[0] += beans[i].getAnnualTaxedIncome()+beans[i].getAnnualNonTaxedIncome();
				subtotal[1] += beans[i].getAnnualNonTaxedIncome();
				subtotal[2] += beans[i].getAnnualTaxedIncome();
			    subtotal[3] += beans[i].getAnnualTax();
%>
		   <tr class="<%=((i%2==1)?"odd":"even")%>" >
		  		...
		  		<td align="right"><%=dollar.format(beans[i].getAnnualTaxedIncome()+beans[i].getAnnualNonTaxedIncome())%></td>
		  		<td align="right"><%=dollar.format(beans[i].getAnnualNonTaxedIncome())%></td>
		  		<td align="right"><%=dollar.format(beans[i].getAnnualTaxedIncome())%></td>
		  		<td align="right"><%=dollar.format(beans[i].getAnnualTax())%></td>
		  </tr>	
