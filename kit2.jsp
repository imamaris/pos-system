...
try {
    ...
    String query =  " SELECT b.pkt_code, b.pkt_desc, d.pmp_skucode, d.pmp_default_name, if(inventory.balance  is not null, inventory.balance, 0) as imbang " +
                    ...
                    " left join (select f.piv_productid, sum(f.piv_in) - sum(f.piv_out) as balance from product_inventory f  where f.piv_status = 100  and f.piv_trxdate <= ' "+tanggal+" '  and f.piv_owner = '"+lokasi+"'  group by f.piv_productid ) as inventory on inventory.piv_productid = d.pmp_productid " +
