...
public class InventoryBroker extends DBTransactionBroker
{

    protected InventoryBroker(Connection con)
    {
        super(con);
    }
    ...
    protected boolean getValidateVerifyInAndVoidApprove(String sku_kode, String lokasi){        
        ...
        try {
                ...
                if(hasil){
                    ...
                    String qVerifyInVoid = "SELECT * from product_inventory where piv_owner='" + lokasi + "' and (piv_trxtype in('STAO','STAI') AND piv_status in (100, 90)) AND piv_status != 40 AND piv_skucode='" + sku_kode + "'";
    ...
    protected String getAvailableLoanOut(String sku_kode, String lokasi, String target)
    throws MvcException
  {
    String balance = "";
    try
    {
      String qry_balance = "SELECT sklo.piv_trxdocno, sklo.piv_skucode, (IFNULL(SUM(sklo.piv_out),0)-IFNULL(SUM(skli.piv_in),0)) AS balance FROM (SELECT * FROM product_inventory WHERE piv_owner_remark='1' AND piv_status=100 AND piv_status != 40 AND piv_status != 40 AND piv_trxtype='SKLO' AND piv_skucode='" + sku_kode + "' AND piv_owner='" + lokasi + "' AND piv_target='" + target + "') AS sklo " + "LEFT JOIN (SELECT * FROM product_inventory WHERE piv_owner_remark='1' AND piv_status=100 AND piv_status != 40 AND piv_trxtype='SKLI' AND piv_skucode='" + sku_kode + "') AS skli " + "ON sklo.piv_skucode=skli.piv_skucode AND sklo.piv_trxdocno=SUBSTR(skli.piv_owner_remark FROM 2 FOR 16) " + "GROUP BY sklo.piv_skucode";
      
      this.stmt = getConnection().prepareStatement(qry_balance);
      this.rs = this.stmt.executeQuery();
      if (this.rs.last())
      {
        System.out.println(this.rs.getString(3));
        balance = "true|" + this.rs.getString(3) + "|" + this.rs.getString(1);
      }
      else
      {
        qry_balance = "SELECT sklo.piv_trxdocno, sklo.piv_skucode, (IFNULL(SUM(sklo.piv_out),0)-IFNULL(SUM(skli.piv_in),0)) AS balance FROM (SELECT * FROM product_inventory WHERE piv_status=100 AND piv_status != 40 AND piv_trxtype='SKLO' AND piv_skucode='" + sku_kode + "' AND piv_owner='" + lokasi + "' AND piv_target='" + target + "') AS sklo " + "LEFT JOIN (SELECT * FROM product_inventory WHERE piv_status=100 AND piv_status != 40 AND piv_trxtype='SKLI' AND piv_skucode='" + sku_kode + "') AS skli " + "ON sklo.piv_skucode=skli.piv_skucode AND sklo.piv_trxdocno=SUBSTR(skli.piv_owner_remark FROM 2 FOR 16) " + "GROUP BY sklo.piv_skucode";
        
        this.stmt = getConnection().prepareStatement(qry_balance);
        this.rs = this.stmt.executeQuery();
        if (this.rs.last())
        {
          System.out.println("Else AvailableLoanOut---" + this.rs.getString(3));
          balance = "false|" + this.rs.getString(3) + "|" + this.rs.getString(1);
        }
        else
        {
          balance = "false|" + String.valueOf(0);
        }
      }
    }
    catch (Exception ex)
  ...
  protected boolean cekOutstandingLO(String sku_kode, String lokasi)
    throws MvcException, InterruptedException
  {
    boolean hasil = false;
    try
    {
      String query = " SELECT piv_trxtype FROM product_inventory WHERE piv_owner='" + lokasi + "'" + " AND (piv_trxtype = 'SKLO' and piv_status in (90))" + " AND piv_skucode = '" + sku_kode + "'";
      
      this.stmt = getConnection().prepareStatement(query);
      this.rs = this.stmt.executeQuery();
      if (this.rs.last())
      {
        System.out.println("cekOutstandingLO (com.ecosmosis.orca.inventory.InventoryBroker.java) : " + this.rs.getString(1));
        if (this.rs.getRow() > 0) {
          hasil = true;
        } else {
          hasil = false;
