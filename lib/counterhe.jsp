...
function calcUnit(i, obj) {
	...
	if (isNaN(unit)){
        ...
        foc = eval("thisform.Foc_" + i ).value;
	price = eval("thisform.iprice_" + i ).value;                
	rate = eval("thisform.irate_" + i).value;
        
        // berikan warning, dan tidak bisa lanjut
        
        iqty1 = eval("thisform.iqty_" + i ).value;   
  
	if (iqty1 - unit < 0 ) {
		alert("<i18n:label code="Quantity Issue is greater than Stock Balance Quantity (Manual Invoice)"/>");
		obj.value = "";
		obj.focus();
		return;
	}  

        var iqty2 = eval("thisform.iqty_" + i );        
...
function calcUnitForce(i, obj) {
	...
	// bypass
        if (iqty1 - unit < 0 ) {
		alert("<i18n:label code="Stock Balance Quantity is less than 0 (Manual Invoice)"/>");
		obj.value = "";
		obj.focus();
		return;
	}  
...
function calcUnitForceReturn(i, obj) {
	var unit = obj.value;
	var thisform = document.frmSalesOrder;
...
function calcUnitFoc2SIR(i, obj) {
	var unit = obj.value;
	var thisform = document.frmSalesOrder;
...
function calcUnitFoc3(i, obj) {
	...
	price = eval("thisform.iprice_" + i ).value;
        ...
        amount = eval("thisform.Amt_" + i ).value;

        // diskon Rp
        var foc = eval("thisform.Foc_"+i);        
        foc.value = foc == 0 ? "0.00"  : decFormat( (price * rate * qty) - amount );        
               
        // diskon %
        foc2 = eval("thisform.Foc_" + i ).value;
        
        var disc = eval("thisform.Disc_"+i);        
        disc.value = disc == 0 ? "0.00"  : decFormat(  (((price * rate * qty) - amount) / (price * rate * qty))  *    100 );              

        var total = eval("thisform.Amt_"+i);      
        // total.value = total == 0 ? ""  :formatAmount((price * rate * qty) - foc2);  
        // decFormat(ProductList[idx].price * unit);
        
        total.innerText = total == 0 ? ""  :formatAmount((price * rate * qty) - foc2);  
        // total.innerText = total == 0 ? ""  :decFormat((price * rate * qty) - foc2);  
        // decFormat(ProductList[idx].price * unit);
...
function calcUnitFoc3SIR(i, obj) {
	var unit = obj.value;
	var thisform = document.frmSalesOrder;
...
function calcGrandTotalSIR() {
	...
	Total.innerText = total == 0 ? "0.00" : formatAmount(total);  //decFormat(total);
        totalOrder = total == 0 ? 0 : total;  //decFormat(total);
	...
	*/
	...
	orderPayment = total == 0 ? total : (total - discount);      
        ...
        if(thisform.DeliveryAmount.value > totalOrder){
            addFee.innerText = formatAmount(thisform.DeliveryAmount.value - totalOrder);
            //disabledCheckbox(false);
            Grandtotal.innerText = '0.00';
        }else{
            addFee.innerText = '0.00';
...
function calcAmountPaid(_this, x)  {
	var thisform = document.frmSalesOrder;
	paid = 0;
       
        var curr1 = document.getElementById('currency_'+x).value;
        var currency = curr1.replace(/[^\d.]/gi,"");
 
       // var currency = document.getElementById('currency_'+x).value;
       var amount = document.getElementById('amount_'+x).value;
       amount2 = 0;
       amount2 = currency * amount;
  
       document.getElementById('amount2_'+x).value= decFormat(amount2);
...
function calcVoucherPaid(_this)  {
	var thisform = document.frmSalesOrder;   

       var voucher = document.getElementById('iprice_').value;
       DiscAmount = 0;
       DiscAmount = 1 * voucher;
  
       // document.getElementById('DiscountAmount').value= decFormat(DiscountAmount);
       document.getElementsByName('DiscountAmount').value= decFormat(voucher);
...
function seletCurrencyMode(_this, x)  {
	var thisform = document.frmSalesOrder;
	paid = 0;
...
function updatePaymentBalance() {
	...
	for (var i=0; i < 16 ; i++) { //Updated By Ferdi 2015-03-10
		var amt = eval("thisform.amount_" + i).value;
		paid += amt * 1;
                
		var amt2 = eval("thisform.amount2_" + i).value;
		paid2 += amt2 * 1;
                
	}
	
	//amountPaid.innerText = paid == 0 ? "0.00" : formatAmount(paid); //decFormat(paid);
	amountPaid.innerText = paid2 == 0 ? "0.00" : formatAmount(paid2); //decFormat(paid2);
        
	//var bal = (paid == 0) ? "0.00" : decFormat(replaceValueWithCommas(Grandtotal.innerText) - paid);
	var bal = (paid2 == 0) ? "0.00" : decFormat(replaceValueWithCommas(Grandtotal.innerText) - paid2);
...
function disabledCheckbox(truefalse) {
    //alert("Check");
    var thisform = document.getElementById("check");
    thisform.setAttribute("disabled", truefalse);
}
