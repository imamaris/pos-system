...
function calcUnitFoc(idx, quantity, qty) {
	var unit = qty.value;
	...
	var total = eval("Amt_"+ProductList[idx].ct+"_"+ProductList[idx].id);        
	total.innerText = unit==0?"":formatAmount((ProductList[idx].price * unit2 - unit   ));  
...
function calcGrandTotal() {
	...
	for (var i=0; i < ProductList.length; i++) {
		...
		total += (ProductList[i].price * qty) - focQty;
