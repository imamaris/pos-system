...
/** Update No Bv Cart - Retailer & Staff **/
...
function noBvCart(idx) {
	...
	var qty = eval("thisform.Qty_"+ ProductList[idx].ct + "_" + ProductList[idx].id).value;
	var foc = eval("thisform.Foc_"+ ProductList[idx].ct + "_" + ProductList[idx].id).value;
	var disc = eval("thisform.Disc2_"+ ProductList[idx].ct + "_" + ProductList[idx].id).value;
        
	if (qty.length > 0 || foc.length > 0 || disc.length > 0) {
...
function addToNoBvCart(no, idx) {
	...
	for (var i=0; i<column; i++) {
		...
		switch (i) {
			...
			break;
			case 5:
				data[i].innerHTML = disc;
				data[i].align = "center";
			break;                        
			case 6:
				var out = replacePriceValue(price.innerHTML);
				data[i].innerHTML = formatAmount(parseFloat(out)*qty - disc);
				data[i].align = "right";
			break;
...
function updateNoBvCart(row, idx) {
	...
	var out = replacePriceValue(price.innerHTML);
	y[6].innerHTML = formatAmount(parseFloat(out)*qty - disc);
}
