...
function updateCart(row, idx) {
	...
	var out = replacePriceValue(price.innerHTML)
	y[6].innerHTML = formatAmount(parseFloat(out)*qty - foc);
}
...
function edp_init() {
	...
	for (var i=0; i < ProductList.length; i++) {
			...
			var qty = eval("thisform.Qty_"+ ProductList[i].ct + "_" + ProductList[i].id).value;
                        ...
                        var foc = eval("thisform.Foc_"+ ProductList[i].ct + "_" + ProductList[i].id).value;
...
function updateNoBvCart(row, idx) {
	...
	var out = replacePriceValue(price.innerHTML);
	y[5].innerHTML = formatAmount(parseFloat(out)*qty - foc);
}
