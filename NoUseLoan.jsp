...
try {
    ...
    String query = " SELECT product_pricing.ppi_currency, product_master.pmp_productid, product_master.pmp_catid, product_master.pmp_skucode, product_master.pmp_productcode, replace(product_master.pmp_default_desc,': ','= ') as  pmp_default_desc, product_category_desc.pcd_name, product_pricing.ppi_price * (1 - (product_pricing.ppi_bv1)) as ppi_price, product_pricing.ppi_pricingid,(product_pricing.ppi_bv1 * 100) as disc, product_pricing.ppi_price * (1 - (product_pricing.ppi_bv1)) as endprice, if(inventory.balance  is not null, inventory.balance, 0) as imbang, product_kit.pkt_code, " +
            ...
            "left join (select piv_productid, sum(piv_out) - sum(piv_in) as balance from product_inventory  where piv_status = 100  and piv_trxdate <= '"+tanggal+"' and piv_owner = '"+lokasi+"' and piv_target = '"+customer+"' AND product_inventory.piv_trxtype IN  ('SKLI', 'SKLO')  group by product_inventory.piv_target, product_inventory.piv_productid ) as inventory on inventory.piv_productid = product_master.pmp_productid " + 
            "where pmp_status='A' and ppi_pricecode='RTL' and date(now()) between ppi_startdate and ppi_enddate and date(now()) >= ppi_startdate and pmp_skucode= '"+sku_kode+"' order by product_pricing.ppi_promotional DESC, product_pricing.ppi_startdate DESC, product_pricing.ppi_price DESC limit 1  "; //Updated By Ferdi 2015-06-19
            //"where pmp_status='A' and ppi_pricecode='RTL' and ppi_startdate <= ' "+tanggal+" ' and ppi_enddate >= ' "+tanggal+"' and pmp_skucode= '"+sku_kode+"' order by product_pricing.std_createdate DESC limit 1  "; //Updated By Ferdi 2015-06-19
